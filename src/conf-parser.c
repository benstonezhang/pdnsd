...
static int read_cast_tree(const char *strbuf, ntree_node_t *atree, slist_array *ata, char **errstr, int op);
...
#	define CLEANUP_HANDLER (free_servparm(&server))
	...
	while(isalpha(*p)) {
	  ...
	  case INCLUDE_FILE:
	    ...
	    {
	      char *errmsg;
	      if ((server.inc_tree==NULL)&&((server.inc_tree=ntree_init())==NULL)) OUTOFMEMERROR;
	      if (!read_cast_tree(strbuf,server.inc_tree,&server.alist,&errmsg,C_INCLUDED)) {
	        if(errmsg) {REPORT_ERROR(errmsg); free(errmsg);}
	        else *errstr=NULL;
	        PARSERROR;
	      }
	  ...
	  case EXCLUDE_FILE:
	    ...
	    {
	      char *errmsg;
	      if ((server.exc_tree==NULL)&&((server.exc_tree=ntree_init())==NULL)) OUTOFMEMERROR;
	      if (!read_cast_tree(strbuf,server.exc_tree,&server.alist,&errmsg,C_EXCLUDED)) {
	          if(errmsg) {REPORT_ERROR(errmsg); free(errmsg);}
	          else *errstr=NULL;
	          PARSERROR;
	     }
...
static int read_cast_tree(const char *fn, ntree_node_t *atree, slist_array *ata, char **errstr, int op)
{
  int rv=0;
  FILE *f;
  char *buf;
